...
set rest = $raw_dir/rest
set rest_SBREF = $raw_dir/rest_SBREF
set T1 = $raw_dir/T1
cd $raw_dir
if ( ! -d $dist_PA ) then
...
if ( ! -d $rest ) then
	mv ./A-P_REST_MUITIBAND8_EPI_CMRR_00?? $rest
endif
if ( ! -d $rest_SBREF ) then
	mv ./A-P_REST_MUITIBAND8_EPI_CMRR_SBREF_00?? $rest_SBREF
endif
if ( ! -d $T1 ) then
