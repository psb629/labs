...
{
	subj="GP$nn"
	setA=($setA $dir_stat/$subj/stats.Rew.$subj+tlrc.HEAD'[Rew#2_Coef]')
}
3dttest++ -mask $mask							\
	-setA $setA									\
...
{
	subj="GP$nn"
	setB=($setB $dir_stat/$subj/stats.Rew.$subj+tlrc.HEAD'[Rew#2_Coef]')
}
3dttest++ -mask $mask					\
	-setA $setB							\
...
{
	subj="GP$nn"
	setC=($setC $dir_stat/$subj/stats.Rew.$subj+tlrc.HEAD'[Rew#2_Coef]')
}
cd $dir_output
3dttest++ -mask $mask							\
	-setA $setC									\
