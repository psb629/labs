...
## T1
dir_raw=`find $dir_root -maxdepth 1 -type d -name "$subj_*T1"`
...
## fMRI
dir_raw=`find $dir_root -mindepth 1 -type d -name "$subj_*MRI"`
# Note, 일반적으로 DICOM 파일은 t시간때의 volume(3D) 정보를 담는 하나의 파일로 생성되는거 같음.
...
ii=1
for time in `seq -f "%04g" 1 300`
{
	for slice in `seq -f "%04g" $time 300 18001`
	{
		jj=`printf %06d $ii`
