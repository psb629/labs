...
foreach ii (`seq -f "%02g" 1 2`)
	...
	set gdrive = ~/GoogleDrive
 #	set data_dir = $gdrive/new_preprocessed_data_from_SMC
	set data_dir = ~/S31
	set output_dir = ~/$subj/preprocessed
	if ( ! -d $output_dir ) then
		mkdir -p -m 755 $output_dir
	endif
	set epi = $data_dir/${subj}_${date}_fMRI.nii
	set t1 = $data_dir/${subj}_${date}_T1.nii
	#########################################################
	...
	cd $output_dir
	# ================================ z-flip =================================
	set pname = $data_dir/${subj}_${date}_fMRI.Zflip.nii
	3dLRflip -prefix $pname -Z $epi
	set epi = $pname
	# ================================ tcat =================================
