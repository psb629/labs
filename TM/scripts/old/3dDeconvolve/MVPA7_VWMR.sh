...
set subj_list = (KJW)
set reg_num = Reg12
set stat_dir = {$reg_num}_MVPA7_IM_VWMR	# vibration / working memory / motor / reward
###########################################################################################
...
foreach subj ($subj_list)
	...
	set reg_file = $reg_subj_dir/onset+duration_VWMR.dat
	...
	foreach run (r01 r02 r03)
		...
		3dLSS -input $preproc_subj_dir/pb04.$subj.$run.scale+tlrc.HEAD	\
			-mask $preproc_subj_dir/full_mask.$subj+tlrc.HEAD			\
			-matrix $TM_dir/fMRI_data/stats/Reg12_{*}/$subj/$run.X.xmat.1D		\
