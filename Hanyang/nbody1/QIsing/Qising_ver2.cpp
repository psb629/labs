...
#define File_magnetization "gChgEp0.005Chi13Tr_g-m.dat"	//g,epsilon,chi_X-Y axis.dat
#define File_printf "_g-dE.dat"		//g,epsilon,chi_X-Y axis.dat
#define File_correlator "_gChgEp0.005Chi13Tr_r-C(r).dat"
...
#define def_chi 15	//bond dimension chi
int def_L=2;
double ini_ep=0.005, fin_ep=0.005;
double g_initial=0.0, g_final=1.501, g_gap=0.005;
double h_initial=0.0, h_final=0.0, h_gap=0.01;
...
int main()
{
	/*double a_gap, k;
	printf("#a\t\ta_gap\n");
	for(double a=0.;a<1.5;a+=a_gap)
	{
		k=1./pow(ABS(a-1.0),2.);
		a_gap=((k<100)?(0.05):((1./k<1.e-3)?(1.e-3):(1./k)));
		printf("%.8e\t%.8e\n",a,a_gap);
	}*/
	backup_main();
	return 0;
...
void printf_columnhead()
{
	FILE *magnet, *corr;
	//File_Gamma "_A.dat"
	///File_lambda "_lam.dat"
	//File_magnetization "gChgEp0.005Chi13_g-m.dat"	//g,epsilon,chi_X-Y axis.dat
	magnet=fopen(File_magnetization,"w");
	fprintf(magnet,"#L\tchi\tg\t\th\t\tnorm\t\tsqrt(G(L/2)/norm)\tcount\tre\n");
	fclose(magnet);
	//File_printf "_g-dE.dat"		//g,epsilon,chi_X-Y axis.dat
	//File_correlator "gChgEp0.005Chi13_r-C(r).dat"
...
void printf_magnetization()
{
	...
	int dummy=def_L;
	for(def_L=100;def_L<=200;def_L+=20)
	{
		...
		fprintf(magnet,"%d\t%d\t%.8e\t%.8e\t%.8e\t%.8e\t\t%d\t%d\n",def_L,def_chi,g,h,dummy_norm,pow(dummy_correlation,0.5),count_converge,count_nonconverge);
	}
	def_L=dummy;
	fclose(magnet);
}
void printf_correlator()
{
	...
	for(int r=1;r<def_L;r++)
	{
		...
		fprintf(corr,"%d\t%d\t%.8e\t%.8e\t%.8e\t%d\t%.8e\t%d\t%d\n",def_L,def_chi,g,h,dummy_norm,r,dummy_correlation/dummy_norm,count_converge,count_nonconverge);
	}
	def_L=dummy;
	fclose(corr);
}
void backup_main()
{
	...
	for(g=g_initial;g<g_final;g+=g_gap)
	{
		...
		printf_magnetization();
		unset_table();
		//double k=1./pow(ABS(g-1.0),2.);
		//g_gap=((k<100)?(0.05):((1./k<1.e-3)?(1.e-3):(1./k)));
	}
	unset_operators();
	unset_tensors();
}
void test_1()
{
	...
	for(g=0.0;g<1.21;g+=0.1)
	{
		...
		double norm;
		def_L=16;
		//for(def_L=2;def_L<=1024;def_L*=2)
		{
			norm=expectation_value(Op_identity, Gamma, lambda, 1, def_L);
			printf("g=%.4f, L=%d, norm=%.8e, TEBD=%d\n",g,def_L,norm,count_converge);
		}
		printf("\n");
		unset_table();
	}
...
void test_2()
{
	...
	for(g=0.98;g<1.05;g+=0.02)
	{
		...
		double energy_TNS=TNS_ENERGY(backup_s[0][0],epsilon);
		double energy_exact=exact_energy(1024);
		double norm, correlation;
		def_L=128;
		printf("#g\t\tr\tC(r)\n");
		for(int r=1;r<def_L;r+=3)
		{
			norm=expectation_value(Op_identity, Gamma, lambda, 1, def_L);
			correlation=expectation_value(Op_sigmaZ, Gamma, lambda, 2, r);
			printf("%.4e\t%d\t%.8e\n",g,r,correlation/norm);
		}
		unset_table();
	}
	unset_operators();
...
void test_3()
{
	...
	for(g=0.0;g<1.51;g+=g_gap)
	{
		...
		double norm, correlation;
		int r;
		printf("#L\tg\t\tchi\tnorm\t\tC(L/2)/norm\tL/2\n");
		for(def_L=20;def_L<=120;def_L+=20)
		{
			r=(int)(def_L*0.5);
			norm=expectation_value(Op_identity, Gamma, lambda, 1, def_L);
			correlation=expectation_value(Op_sigmaZ, Gamma, lambda, 2, r);
			printf("%d\t%.8e\t%d\t%.8e\t%.8e\t%d\n",def_L,g,def_chi,norm,correlation/norm,r);
		}
		unset_table();
		//weighting in the vicintiy of a singular point//
		//double k=1./pow(ABS(g-1.0),2.);
		//g_gap=((k<100)?(0.05):((1./k<1.e-3)?(1.e-3):(1./k)));
		/////////////////////////////////////////////////
	}
