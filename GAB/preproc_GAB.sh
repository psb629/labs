...
set subj = GAB01
#=============================================
set root_dir = $HOME/Desktop/*_${subj}
set raw_dir = $root_dir/HEAD_{*}
set output_dir = $root_dir/preprocessed
...
set r01 = $raw_dir/r01
set r01_SBREF = $raw_dir/r01_SBREF
set T1 = $raw_dir/T1
cd $raw_dir
if ( ! -d $dist_PA ) then
	mv ./DISTORTION_CORR_64CH_PA_00?? $dist_PA
endif
if ( ! -d $dist_AP ) then
	mv ./DISTORTION_CORR_64CH_PA_POLARITY_INVERT_TO_AP_00?? $dist_AP
endif
if ( ! -d $r01 ) then
	mv ./RUN1_MUITIBAND8_EPI_CMRR_00?? $r01
endif
if ( ! -d $r01_SBREF ) then
	mv ./RUN1_MUITIBAND8_EPI_CMRR_SBREF_00?? $r01_SBREF
endif
if ( ! -d $T1 ) then
	mv ./T1_MPRAGE_SAG_0_7ISO_00?? $T1
endif
#=============================================
## Convert *.IMA files to *.BRIK
cd $dist_PA
...
foreach run ($run_list)
	...
	3dAllineate -base $subj.anat.unifize+tlrc \
		...
		-mast_dxyz $res   -prefix rm.epi.nomask.$subj.$run # $res는 original data의 resolution과 맞춤.
