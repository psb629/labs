...
stat='4s_shifted'
list_subj=(`ls $dir_root/GLM.reward.$stat.SSKim | grep '^GL[0-9][0-9]'`)
...
##===========================================##
# Move-Stop
cd $dir_root/GLM.Move-Stop.SSKim
foreach subj ($list_subj)
	...
	tstat="$subj/$subj.Move-Stop_GLT#0_Tstat.$coord.nii"
	dof=`3dinfo -verb $tstat | grep -o -E 'statpar = [0-9]+' | grep -o -E '[0-9]+'`
	TtoZ --t_stat_map=$tstat --dof=$dof --output_nii="$subj/$subj.Move-Stop.z_score_map.nii"
end

# Reward
 #cd $dir_root/GLM.reward.$stat.SSKim
 #foreach subj ($list_subj)
