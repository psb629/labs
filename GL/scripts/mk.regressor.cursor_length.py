...
## ========================================================= ##
dir_output = join(dir_reg, 'AM/cursor_length/shift=%d'%time)
...
txt_cursor = np.loadtxt(
        join(dir_reg, '%s.cursor_length.txt'%subj),
...
for r in range(nrun): 
    for t, (onset, cursor, c) in enumerate(zip(txt_onset[r], txt_cursor[r], txt_cond[r])):
        if not c in cc:
            continue
        reg[r,t] = '%.3f*%s'%(onset+time,cursor)
 #print(reg)
reg_cut = []
for rr in reg:
    tmp = rr[rr!='']
...
if apart=='n':
    np.savetxt(
        join(dir_output,'%s.cursor.shift=%d.%s.rall.txt'%(subj,time,cond)),
        X=reg_cut,
    ...
    for ii, rr in enumerate(reg_cut):
        np.savetxt(
            join(dir_output,'%s.cursor.shift=%d.%s.r%02d.txt'%(subj,time,cond,ii+1)),
            X=rr,
            fmt='%s', delimiter=' ', newline=' '
        )
## ========================================================= ##
