...
set list_subj = (S38 S46 S47 S48)
set list_subj = (S38)
set list_date = (220222 220218 220221 220221)
set list_date = (211224)
set coord = orig
...
foreach ii (`seq -f "%02g" 1 $#list_subj`)
	...
	set dir_data = $dir_root/${subj}_${date}_MRI

	set t1 = $dir_root/preprocessed/$subj/${subj}_T1.nii
	set epi = $dir_root/preprocessed/$subj/${subj}_fMRI.nii
	#########################################################
	...
	# ========================= T1 data : 366 files =========================
	if ( ! -f $t1 ) then
		set raw_T1 = $dir_data/${subj}_${date}_T1
		dcm2niix_afni -f "${subj}_T1" -o $dir_output -p y -z n $raw_T1

		## added the follow line since S19, because of the unexpected suffix "_real"
		cd $dir_output
		foreach ext ('json' 'nii')
			mv ${subj}_T1_real.$ext ${subj}_T1.$ext
		end
	endif
	...
	if ( ! -f $epi ) then
		set raw_fMRI = $dir_data/${subj}_${date}_fMRI
		...
		foreach t_ini ($set_time)
			...
			foreach n ($set_data)
				...
				cp $raw_fMRI/$subj.dcm$n.dcm temp$n_prime.dcm
			end
			set t = `printf %03d $t_ini`
			dcm2niix_afni -o $dir_output -s y -z n -f "${subj}_func$t" $dir_output
			rm $dir_output/*.dcm
		end
		3dTcat -tr $TR -prefix $dir_output/temp $dir_output/${subj}_func*.nii
		rm $dir_output/${subj}_func*.nii
		3dAFNItoNIFTI -prefix $epi $dir_output/temp+orig
		rm $dir_output/*
	endif
	#########################################################
	########
