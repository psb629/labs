...
foreach nn ($nn_list)
	...
	foreach gg (GA GB)
		...
		foreach rr (`count -digits 2 1 6`)
			...
			fin_res=$output_dir/$subj.bp_demean.errts.MO.RO.r$rr.nii.gz
			if [ -f $fin_res ]; then
				continue
			fi
			## move main files to the temp_dir directory
			...
			3dDeconvolve -input $temp_dir/$pb02 \
						...
						-stim_label 1 MO \
	 					-stim_times_AM2 2 $behav_dir/regressors/rewards/$subj.r${rr}rew1000.GAM.1D 'SPMG2' \
						-stim_label 2 rwdtm \
						-stim_file 3 "$fmri_dir/preproc_data/$nn/motion_demean.$subj.r$rr.1D[0]" -stim_base 3 -stim_label 3 roll \
						-stim_file 4 "$fmri_dir/preproc_data/$nn/motion_demean.$subj.r$rr.1D[1]" -stim_base 4 -stim_label 4 pitch \
						-stim_file 5 "$fmri_dir/preproc_data/$nn/motion_demean.$subj.r$rr.1D[2]" -stim_base 5 -stim_label 5 yaw \
						-stim_file 6 "$fmri_dir/preproc_data/$nn/motion_demean.$subj.r$rr.1D[3]" -stim_base 6 -stim_label 6 dS \
						-stim_file 7 "$fmri_dir/preproc_data/$nn/motion_demean.$subj.r$rr.1D[4]" -stim_base 7 -stim_label 7 dL \
						-stim_file 8 "$fmri_dir/preproc_data/$nn/motion_demean.$subj.r$rr.1D[5]" -stim_base 8 -stim_label 8 dP \
						-x1D $output_dir/$subj.X.MO.RO.r$rr.1D \
						-xjpeg $output_dir/$subj.X.MO.RO.r$rr.jpg \
						-x1D_uncensored $output_dir/$subj.X.nocensor.MO.RO.r$rr.1D \
						-errts $output_dir/$subj.errts.MO.RO.r$rr.nii.gz \
						-bucket $output_dir/$subj.stats.MO.RO.r$rr.nii.gz
