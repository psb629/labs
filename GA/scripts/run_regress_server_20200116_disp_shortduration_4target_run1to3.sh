...
foreach subj ($subjlist)
    ...
    cat ${data_dir}/motion_demean.$subj.r01.1D \
        ${data_dir}/motion_demean.$subj.r02.1D \
        ${data_dir}/motion_demean.$subj.r03.1D \
        > ${data_dir}/motion_demean.${subj}.run1to3.1D
    cat ${data_dir}/motion_{$subj}.r01_censor.1D \
        ${data_dir}/motion_{$subj}.r02_censor.1D \
        ${data_dir}/motion_{$subj}.r03_censor.1D \
        > ${data_dir}/motion_${subj}.censor.run1to3.1D
    ...
    3dDeconvolve -input $data_dir/pb02.$subj.r01.volreg+tlrc.HEAD     \
        ...
        -censor $data_dir/motion_${subj}.censor.run1to3.1D          \
        ...
        -stim_file 5 ${data_dir}/motion_demean.$subj.run1to3.1D'[0]' -stim_base 5 -stim_label 5 roll   \
        -stim_file 6 ${data_dir}/motion_demean.$subj.run1to3.1D'[1]' -stim_base 6 -stim_label 6 pitch  \
        -stim_file 7 ${data_dir}/motion_demean.$subj.run1to3.1D'[2]' -stim_base 7 -stim_label 7 yaw    \
        -stim_file 8 ${data_dir}/motion_demean.$subj.run1to3.1D'[3]' -stim_base 8 -stim_label 8 dS     \
        -stim_file 9 ${data_dir}/motion_demean.$subj.run1to3.1D'[4]' -stim_base 9 -stim_label 9 dL     \
        -stim_file 10 ${data_dir}/motion_demean.$subj.run1to3.1D'[5]' -stim_base 10 -stim_label 10 dP     \
