...
foreach n (`count -digit 1 1 $#roi`)
	# =======================================================
	3dDeconvolve -input "CorrZ.$roi[$n].GD.n${#subj_list}.rest.WM+tlrc" \
		-mask $full_mask
		-num_stimts 1 -stim_file 1 rew_tot_n${#subj_list}.1D -stim_label 1 rew_tot \
		-fout -tout -polort 0 -bucket Corr.$roi.GD.rew_tot.rest.WM_n$#subj_list

	set pname = $output_dir/Corr.$roi[$n].GD.rew_tot.rest.WM_n${#subj_list}.new
	3dTcorr1D -prefix $pname \
		-float -mask $full_mask \
		"CorrZ.$roi[$n].GD.n$#subj_list.rest.WM+tlrc" rew_tot_n${#subj_list}.1D
	3dAFNItoNIFTI Corr.$roi[$n].GD.rew_tot.rest.WM_n${#subj_list}.new+tlrc \
		$roi[$n].GA.rew_tot.rest.WM_n${#subj_list}.new
	# =======================================================
